...
async def get_stock_data(symbol: str, retries: int = 3) -> Dict:
    ...
    for attempt in range(retries):
        try:
            ...
            if hist.empty or len(hist) < 14:  # Min data para RSI
            ...
            logger.info(f"✅ {symbol}: OK ({indicadores['precio_actual']})")
            return indicadores
            ...
            if "429" in str(e) or "Too Many Requests" in str(e):
                wait = (2 ** attempt) + np.random.uniform(0, 1)  # Exponential backoff
